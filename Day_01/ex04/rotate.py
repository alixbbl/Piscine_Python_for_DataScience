...
def ft_rotate(img_array, angle):
    ...
    for y in range(height):
        for x in range(width):
            ...
            new_x = int(
                (x - c_x) * math.cos(angle_rad) - (y - c_y) * math.sin(angle_rad) + c_x
            )
            new_y = int(
                (x - c_x) * math.sin(angle_rad) + (y - c_y) * math.cos(angle_rad) + c_y
            )
...
def ft_transpose(path_img, start_row, end_row, start_col, end_col, angle):
    ...
    img_cut = img_load.crop(
        (start_col, start_row, end_col, end_row)
    )  # attention au tuple pour crop()
    img_gray_cut = img_cut.convert("L")
    img_gray_cut_array = np.array(img_gray_cut)
    gray_cut_array_reshaped = np.expand_dims(
        img_gray_cut_array, axis=-1
    )  # premier ajout de la 3e dimension

    print(
        f"The shape of image is: {gray_cut_array_reshaped.shape} or \
    ...
    print(rotated_img)
    return rotated_img
    # Il faut a nouveau ajouter la 3dim car la rotation se fait sur un 2D array,
    # la 3d est perdue
