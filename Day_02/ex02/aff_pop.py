...
def filter_country_data_to_list(data, years_list, country):
	...
	"""
	This function filters the requested country data, then it shortens the list to
	the mandatory length of the graph.
	Returns a list of data (uses iloc and tolist()).
	"""
	country_data = data[data['country'] == country] # operation de filtrage
	# On coupe les data en fonction de la longueur imposee de l'axe des années
...
def plot_data(years_list, french_data, country_name, country_data):
	...
	# Modifier l'affichage par défaut de la population en ordonnée :
...
def main():
	...
	# Faire cette recherche clef-valeur va aller chercher TOUTES les data relatives
	# au champs France, cela implique aussi les années!
	# Si on souhaite juste une ligne ou colonne, utiliser loc ou iloc
	...
	# Syntaxe permettant de tronquer la premiere colonne et de supprimer son mom
	...
	country_data_list = filter_country_data_to_list(data_all, years_list, selected_country)
